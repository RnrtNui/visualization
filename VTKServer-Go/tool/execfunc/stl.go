...
//BackSTLInfo return stl info
func BackSTLInfo(stlPath string) (*StlData, error) {
	facet, err := ParseSTL(stlPath)
	if len(facet) == 0 || err != nil {
		beego.Debug("facet:", len(facet), err)
		return nil, err
	}
	stldata := &StlData{}
	for i := 0; i < len(facet); i++ {
		stldata.POINTS = append(stldata.POINTS, facet[i].Vertices[0].X, facet[i].Vertices[0].Y, facet[i].Vertices[0].Z)
		stldata.POINTS = append(stldata.POINTS, facet[i].Vertices[1].X, facet[i].Vertices[1].Y, facet[i].Vertices[1].Z)
		stldata.POINTS = append(stldata.POINTS, facet[i].Vertices[2].X, facet[i].Vertices[2].Y, facet[i].Vertices[2].Z)
	}
	for i := 0; i < 3*len(facet); i++ {
		if i%3 == 0 {
			stldata.CELLS = append(stldata.CELLS, 3)
		}

		stldata.CELLS = append(stldata.CELLS, i)
	}
	return stldata, nil
}
