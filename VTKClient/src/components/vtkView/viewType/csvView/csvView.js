...
export default class csvView extends Component {
    ...
    render() {
        let { boxBgColor, model, xLength, yLength, zLength, screen } = this.state;
        let { show, state} = this.props;
        // let { attribute, moveStyle } = store.getState()
        // if (keydown === "R") {//回到中心
        //     if (model.renderer) {
        //         model.renderer.resetCamera()
        //         model.renderWindow.render();
        ...
        //改变鼠标事件
        changeManipulators(model, state.moveStyle);
        ...
        return (
            <div>
                <Draggable handle=".handle"
                    ...
                    grid={[1, 1]}
                    scale={1}>
                    <div style={{ display: state.attribute, position: "absolute", zIndex: "999", padding: "20px" }}>
