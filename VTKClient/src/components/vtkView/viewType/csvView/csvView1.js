...
import { Rendering, Screen, reassignManipulators, changeManipulators, showBoundRuler } from "../../common/index"
...
export default class csvView extends Component {
    ...
    result = () => {
        ...
        if (data.type === ".csv") {
            ...
            //     axios.post(global.baseUrl + "/csvData", {
            //         dataType: ".csv",
            //         POINTS: mapData,
            //         fileName: name
            //     })
    ...
    render() {
        let { boxBgColor, model } = this.state;
        let { show, state } = this.props;
        let { moveStyle, screen, ruler, attribute, scale, ranging } = state;
        let scaleOpc = 0;
        if (scale === true) {
            scaleOpc = 1;
        } else {
        ...
        if (model.renderer) {
            ...
            showBoundRuler(ruler, model, this.container, vtk(model.actor.getMapper().getInputData().getState()), this.props, ranging); //刻度标尺
        }
        ...
        return (
            <div>
                ...
                <div style={{ width: "8%", minWidth: "100px", minWidth: "8%", position: "absolute", left: "5%", bottom: "5%", opacity: scaleOpc, borderBottom: "2px solid #fff" }}>
                    <div style={{ height: "1vh", position: "relative" }}>
                        <div style={{ position: "absolute", left: "0", borderLeft: "2px solid #fff", height: '100%' }}>
                            <span style={{ position: "absolute", color: "#fff", display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>0</span>
                        </div>
                        <div style={{ position: "absolute", left: "50%", borderLeft: "2px solid #fff", height: '100%' }}>
                            <span style={{ position: "absolute", color: "#fff", display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>50</span>
                        </div>
                        <div style={{ position: "absolute", right: "0", borderRight: "2px solid #fff", height: '100%' }}>
                            <span style={{ position: "absolute", color: "#fff", display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>100</span>
                        </div>
                    </div>
                </div>
            </div>
